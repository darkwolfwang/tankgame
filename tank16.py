...
class Bullet():
    ...
    def move(self):
        if self.direction == 'U':
            if self.rectU.top>0:
                self.rectU.top-=self.speed
            else:
                #修改状态
                self.live=False
        elif self.direction == 'D':
            if self.rectD.top+self.rectD.height<SCREEN_HEIGHT:
                self.rectD.top+=self.speed
            else:
                # 修改状态
                self.live = False
        elif self.direction == 'L':
            if self.rectL.left>0:
                self.rectL.left-=self.speed
            else:
                # 修改状态
                self.live = False
        elif self.direction == 'R':
            if self.rectR.left+self.rectR.width<SCREEN_WIDTH:
                self.rectR.left+=self.speed
            else:
                # 修改状态
                self.live = False
